...
RSpec.describe GetBuildingsEntranceImageUrlController, :type => :controller do
  ...
  describe "GET get_buildings_entrance_image_url" do
    ...
    it "正常系（日本語）" do
      post :get_buildings_entrance_image_url, :lang => "ja", :building_id => "9686"
      expect(response.status).to eq(200)
      ...
      expect(response.body).to have_json_path("response/info/building_id")
      expect(response.body).to have_json_path("response/info/building_name")
      ...
      expect(response.body).to have_json_path("response/info/duty_free_count")
      expect(response.body).to have_json_path("response/info/concierge_count")
      expect(response.body).to have_json_path("response/info/entrance_info")
      expect(response.body).to have_json_path("response/info/entrance_info/0/entrance_image")
      expect(response.body).to have_json_path("response/info/entrance_info/0/shop_id")
      
    end
    
    it "正常系（中国語）" do
      post :get_buildings_entrance_image_url, :lang => "zh-cn", :building_id => "9686"
      expect(response.status).to eq(200)
